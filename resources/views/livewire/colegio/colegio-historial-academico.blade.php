    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach ($estudiantes as $est)
            <div wire:click="seleccionarEstudiante({{ $est->id }})" class="bg-white border-2 hover:border-2 hover:border-blue-500 dark:bg-gray-900 p-4 rounded-xl shadow-md cursor-pointer hover:shadow-lg transition-all">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-300">Promedio: {{ $est->promedio($est->estudiantesGrupos->first()->grupo->id) ?? 'sin notas' }} </p>
            ...
            <div class="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-600 dark:text-gray-300">Promedio general: <span class="font-semibold">{{ $estudianteSeleccionado->promedio($estudianteSeleccionado->estudiantesGrupos->first()->grupo->id) ?? 'sin notas' }}</span></p>
                    </div>
                ...
                {{-- Selector de período --}}
                <div class="mb-4">
                    <select wire:model.live="periodo_id" class="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 text-sm text-gray-800 dark:text-white">
                ...
                <div class="mt-6">
                    <button wire:click="descargarNotas" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm">
